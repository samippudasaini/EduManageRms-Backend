...
public class UserController {
    ...
    @PostMapping public ResponseEntity<?> create(@RequestBody Map<String, Object> body) {
        var u = User.builder()
                ...
                .password(encoder.encode((String) body.get("password")))
                .type((String) body.get("type"))
                .facultyDetailId((String) body.get("facultyDetailId"))
                .canAttendance(asBoolean(body.get("canAttendance")))
                .canAssignment(asBoolean(body.get("canAssignment")))
                .canExam(asBoolean(body.get("canExam")))
                .canResult(asBoolean(body.get("canResult")))
                .canStudent(asBoolean(body.get("canStudent")))
                .build();
        return ResponseEntity.ok(repo.save(u));
    }
    ...
    @PutMapping("/{id}") public ResponseEntity<?> update(@PathVariable Long id, @RequestBody Map<String, Object> body) {
        return repo.findById(id).map(u -> {
            ...
            if (body.containsKey("type")) u.setType((String) body.get("type"));
            if (body.containsKey("facultyDetailId")) u.setFacultyDetailId((String) body.get("facultyDetailId"));
            if (body.containsKey("canAttendance")) u.setCanAttendance(asBoolean(body.get("canAttendance")));
            if (body.containsKey("canAssignment")) u.setCanAssignment(asBoolean(body.get("canAssignment")));
            if (body.containsKey("canExam")) u.setCanExam(asBoolean(body.get("canExam")));
            if (body.containsKey("canResult")) u.setCanResult(asBoolean(body.get("canResult")));
            if (body.containsKey("canStudent")) u.setCanStudent(asBoolean(body.get("canStudent")));
            return ResponseEntity.ok(repo.save(u));
        }).orElse(ResponseEntity.notFound().build());
    }
